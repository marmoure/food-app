...
src/
  ...
  storage/    AppData schema + parser, StorageAdapter (browser + project-folder sync), data report,
...
server/       Vite plugin: GET/PUT /api/data, saves data/sunday-kitchen.json + data/status.md.
data/         The user's saved data (git-ignored). Created on the first change in the app.
...
- All storage goes through `StorageAdapter`. `syncedAdapter` keeps a browser copy plus the project copy that
  `server/data-api.ts` (Vite plugin, dev + preview) writes to `data/`. The newest copy wins on load.
- **To see what the user ticked, bought or froze, read `data/status.md`** (readable) or
  `data/sunday-kitchen.json` (raw). Don't edit them while the app is open: the app overwrites them on the
  next change. The whole `data/` folder is git-ignored and personal.
